...
void init_figure_eight(SystemState &state)
{
    ...
    state.bodies[0].x = 0.97000436;
    state.bodies[0].y = -0.24308753;
    state.bodies[0].z = 0;
    state.bodies[1].x = -0.97000436;
    state.bodies[1].y = 0.24308753;
    state.bodies[1].z = 0;
    state.bodies[2].x = 0.0;
    state.bodies[2].y = 0.0;
    state.bodies[2].z = 0;

    double vx = 0.93240737 / 2.0;
    double vy = 0.86473146 / 2.0;
    state.bodies[0].Vx = vx;
    state.bodies[0].Vy = vy;
    state.bodies[1].Vx = vx;
    state.bodies[1].Vy = vy;
    state.bodies[2].Vx = -2 * vx;
    state.bodies[2].Vy = -2 * vy;
...
double compute_energy(const SystemState &state)
{
    ...
    for (int i = 0; i < state.N; i++)
    {
        double v2 = state.bodies[i].Vx * state.bodies[i].Vx + state.bodies[i].Vy * state.bodies[i].Vy + state.bodies[i].Vz * state.bodies[i].Vz;
...
double compute_angular_momentum_z(const SystemState &state)
{
    double Lz = 0.0;
    for (int i = 0; i < state.N; i++)
        Lz += state.bodies[i].m * (state.bodies[i].x * state.bodies[i].Vy - state.bodies[i].y * state.bodies[i].Vx);
    return Lz;
}
